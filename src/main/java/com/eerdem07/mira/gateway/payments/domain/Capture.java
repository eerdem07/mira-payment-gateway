...
import com.eerdem07.mira.gateway.payments.domain.exception.InvalidCaptureAmountException;
import com.eerdem07.mira.gateway.payments.domain.exception.InvalidCaptureStateTransitionException;
...
public class Capture {
    ...
    private Capture(
        ...
        this.posCaptureId = posCaptureId;
    ...
    public void markProcessing(Instant now) {
        ...
        ensureStatusIn(CaptureStatus.PROCESSING, CaptureStatus.CREATED);
    ...
    public void markSucceeded(
        ...
        Objects.requireNonNull(now, "now must not be null");
        ensureStatusIn(CaptureStatus.SUCCEEDED, CaptureStatus.PROCESSING);

        this.status = CaptureStatus.SUCCEEDED;
        this.posCaptureId = requireText(posCaptureId, "posCaptureId must not be blank");
        this.posReference = posReference;
    ...
    public void markFailed(String failureCode, String failureMessage, Instant now) {
        markFailed(failureCode, failureMessage, null, null, now);
    }
    ...
    public void markFailed(
        ...
        Objects.requireNonNull(now, "now must not be null");
        ensureStatusIn(CaptureStatus.FAILED, CaptureStatus.PROCESSING);
    ...
    public void cancel(Instant now) {
        Objects.requireNonNull(now, "now must not be null");
        ensureStatusIn(CaptureStatus.CANCELED, CaptureStatus.CREATED);

        this.status = CaptureStatus.CANCELED;
        this.canceledAt = now;
        this.updatedAt = now;
    }

    private void ensureStatusIn(CaptureStatus targetStatus, CaptureStatus... allowedStatuses) {
        for (CaptureStatus allowedStatus : allowedStatuses) {
            if (this.status == allowedStatus) {
                return;
            }
        }

        throw new InvalidCaptureStateTransitionException(this.status, targetStatus);
    }
